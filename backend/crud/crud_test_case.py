...
class CRUDTestCase:
    def get_test_case(self, db: Session, test_case_id: int):
        """
        根据ID从数据库中获取单个测试用例
        """
    ...
    def get_test_cases(self, db: Session, skip: int = 0, limit: int = 100) -> List[test_case_model.TestCase]:
        """
        从数据库中获取测试用例列表
        """
        return (
    ...
    def create_test_case(self, db: Session, test_case: test_case_schema.TestCaseCreate) -> test_case_model.TestCase:
        """
        在数据库中创建一个新的测试用例
        """
    ...
    def update_test_case(self, db: Session, test_case_id: int, test_case: test_case_schema.TestCaseUpdate) -> Optional[test_case_model.TestCase]:
        """
        更新数据库中的测试用例
        """
        db_test_case = self.get_test_case(db, test_case_id)
        if db_test_case:
    ...
    def reorder_test_cases(self, db: Session, test_case_ids: List[int]) -> List[test_case_model.TestCase]:
        """根据提供的ID列表重新排序测试用例"""
        updated_test_cases = []
    ...
    def update_test_case_priority(self, db: Session, test_case_id: int, priority: int) -> Optional[test_case_model.TestCase]:
        """
        更新数据库中测试用例的优先级
        """
        db_test_case = self.get_test_case(db, test_case_id)
    ...
    def delete_test_case(self, db: Session, test_case_id: int) -> Optional[test_case_model.TestCase]:
        """
        从数据库中删除一个测试用例
        """
        db_test_case = self.get_test_case(db, test_case_id)
    ...
    def delete_test_cases(self, db: Session, test_case_ids: List[int]) -> int:
        """
        批量删除测试用例
        """
        # 使用 in_ 操作符进行批量查询和删除
    ...
    def copy_test_case(self, db: Session, test_case_id: int):
        # 获取源用例
        db_test_case = self.get_test_case(db, test_case_id)
        if not db_test_case:
            return None
